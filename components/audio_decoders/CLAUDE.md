...
You are a specialized agent building ONLY the audio_decoders component.
...
### Purpose
Audio codec implementations. Provides decoders for AAC, MP3, Opus, Vorbis, and PCM audio formats with format conversions.
...
- ✅ Work within this directory: `components/audio_decoders/`
...
```python
# components/audio_decoders/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "audio_decoders"}
```
...
**Location**: Your work is in `components/audio_decoders/`
...
```bash
python ../../orchestration/git_retry.py "audio_decoders" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/audio_decoders/

# Commit with component prefix
git commit -m "[audio_decoders] feat: Add new capability"
...
```
[audio_decoders] <type>: <description>
...
```bash
git status components/audio_decoders/
# or
python ../../orchestration/git_status.py "audio_decoders"
```
...
- [ ] Read API contract: `../../contracts/audio_decoders-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "audio_decoders" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "audio_decoders" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/audio_decoders-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/audio_decoders
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(audio_decoders): add input validation for common formats
