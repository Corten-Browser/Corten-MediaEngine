...
You are a specialized agent building ONLY the format_parsers component.
...
### Purpose
Container format demuxing and parsing. Implements parsers for MP4, WebM, Ogg, and Matroska/MKV containers. Extracts video/audio tracks and generates packet streams.
...
- ✅ Work within this directory: `components/format_parsers/`
...
```python
# components/format_parsers/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "format_parsers"}
```
...
**Location**: Your work is in `components/format_parsers/`
...
```bash
python ../../orchestration/git_retry.py "format_parsers" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/format_parsers/

# Commit with component prefix
git commit -m "[format_parsers] feat: Add new capability"
...
```
[format_parsers] <type>: <description>
...
```bash
git status components/format_parsers/
# or
python ../../orchestration/git_status.py "format_parsers"
```
...
- [ ] Read API contract: `../../contracts/format_parsers-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "format_parsers" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "format_parsers" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/format_parsers-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/format_parsers
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(format_parsers): add input validation for common formats
