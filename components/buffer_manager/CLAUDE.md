...
**Tech Stack**: Rust, bytes, crossbeam
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the buffer_manager component.
...
### Purpose
Memory buffer and cache management. Implements ring buffers for streaming, frame caches for video, audio sample buffers, memory usage tracking, and cache eviction policies.
...
- ✅ Work within this directory: `components/buffer_manager/`
...
```python
# components/buffer_manager/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "buffer_manager"}
```
...
**Location**: Your work is in `components/buffer_manager/`
...
```bash
python ../../orchestration/git_retry.py "buffer_manager" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/buffer_manager/

# Commit with component prefix
git commit -m "[buffer_manager] feat: Add new capability"
...
```
[buffer_manager] <type>: <description>
...
```bash
git status components/buffer_manager/
# or
python ../../orchestration/git_status.py "buffer_manager"
```
...
- [ ] Read API contract: `../../contracts/buffer_manager-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "buffer_manager" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "buffer_manager" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/buffer_manager-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/buffer_manager
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(buffer_manager): add input validation for common formats
