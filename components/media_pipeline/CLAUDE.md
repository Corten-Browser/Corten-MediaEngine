...
**Tech Stack**: Rust, Tokio, crossbeam, parking_lot
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the media_pipeline component.
...
### Purpose
Pipeline orchestration and A/V synchronization. Coordinates source readers, demuxers, decoders, and synchronization. Manages thread pools and pipeline state machine.
...
- ✅ Work within this directory: `components/media_pipeline/`
...
```python
# components/media_pipeline/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "media_pipeline"}
```
...
**Location**: Your work is in `components/media_pipeline/`
...
```bash
python ../../orchestration/git_retry.py "media_pipeline" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/media_pipeline/

# Commit with component prefix
git commit -m "[media_pipeline] feat: Add new capability"
...
```
[media_pipeline] <type>: <description>
...
```bash
git status components/media_pipeline/
# or
python ../../orchestration/git_status.py "media_pipeline"
```
...
- [ ] Read API contract: `../../contracts/media_pipeline-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "media_pipeline" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "media_pipeline" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/media_pipeline-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/media_pipeline
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(media_pipeline): add input validation for common formats
