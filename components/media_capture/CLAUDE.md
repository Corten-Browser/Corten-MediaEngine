...
**Tech Stack**: Rust, platform-specific capture APIs (V4L2, AVFoundation, DirectShow)
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the media_capture component.
...
### Purpose
Device capture APIs. Implements screen capture, camera capture, microphone capture, MediaStream API, and capture constraints handling.
...
- ✅ Work within this directory: `components/media_capture/`
...
```python
# components/media_capture/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "media_capture"}
```
...
**Location**: Your work is in `components/media_capture/`
...
```bash
python ../../orchestration/git_retry.py "media_capture" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/media_capture/

# Commit with component prefix
git commit -m "[media_capture] feat: Add new capability"
...
```
[media_capture] <type>: <description>
...
```bash
git status components/media_capture/
# or
python ../../orchestration/git_status.py "media_capture"
```
...
- [ ] Read API contract: `../../contracts/media_capture-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "media_capture" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "media_capture" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/media_capture-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/media_capture
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(media_capture): add input validation for common formats
