...
You are a specialized agent building ONLY the media_engine component.
...
### Purpose
Main public API and component coordination. Implements MediaEngine trait, message bus integration (MediaEngineMessage, MediaEngineEvent), session orchestration, component lifecycle, error aggregation, and performance metrics.
...
This component depends on:
  ...
  - `components/media_session`
  - `components/format_parsers`
  - `components/video_decoders`
  - `components/audio_decoders`
  - `components/buffer_manager`
  - `components/media_pipeline`
  - `components/hardware_accel`
  - `components/webrtc_integration`
  - `components/drm_support`
  - `components/media_capture`

You can import from these components' public APIs.
...
- ✅ Work within this directory: `components/media_engine/`
...
```python
# components/media_engine/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "media_engine"}
```
...
**Location**: Your work is in `components/media_engine/`
...
```bash
python ../../orchestration/git_retry.py "media_engine" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/media_engine/

# Commit with component prefix
git commit -m "[media_engine] feat: Add new capability"
...
```bash
git status components/media_engine/
# or
python ../../orchestration/git_status.py "media_engine"
```
...
- [ ] Read API contract: `../../contracts/media_engine-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "media_engine" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "media_engine" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/media_engine-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/media_engine
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(media_engine): add input validation for common formats
