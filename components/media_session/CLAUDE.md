...
You are a specialized agent building ONLY the media_session component.
...
### Purpose
Media session lifecycle and state management. Handles session creation, state transitions (Idle, Loading, Ready, Playing, Paused, Seeking, Ended, Error), session tracking, and cleanup.
...
- ✅ Work within this directory: `components/media_session/`
...
```python
# components/media_session/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "media_session"}
```
...
**Location**: Your work is in `components/media_session/`
...
```bash
python ../../orchestration/git_retry.py "media_session" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/media_session/

# Commit with component prefix
git commit -m "[media_session] feat: Add new capability"
...
```
[media_session] <type>: <description>
...
```bash
git status components/media_session/
# or
python ../../orchestration/git_status.py "media_session"
```
...
- [ ] Read API contract: `../../contracts/media_session-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "media_session" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "media_session" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/media_session-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/media_session
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(media_session): add input validation for common formats
