...
**Tech Stack**: Rust, openh264, dav1d, rav1e, vpx, FFmpeg bindings
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the video_decoders component.
...
### Purpose
Video codec implementations. Provides decoders for H.264, H.265, VP8, VP9, and AV1 video codecs with software decode fallback.
...
- ✅ Work within this directory: `components/video_decoders/`
...
```python
# components/video_decoders/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "video_decoders"}
```
...
**Location**: Your work is in `components/video_decoders/`
...
```bash
python ../../orchestration/git_retry.py "video_decoders" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/video_decoders/

# Commit with component prefix
git commit -m "[video_decoders] feat: Add new capability"
...
```
[video_decoders] <type>: <description>
...
```bash
git status components/video_decoders/
# or
python ../../orchestration/git_status.py "video_decoders"
```
...
- [ ] Read API contract: `../../contracts/video_decoders-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "video_decoders" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "video_decoders" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/video_decoders-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/video_decoders
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(video_decoders): add input validation for common formats
