...
**Tech Stack**: Rust, webrtc crate, RTP/RTCP libraries
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the webrtc_integration component.
...
### Purpose
WebRTC media stream handling. Provides WebRTC encoder/decoder wrappers, RTP packetization/depacketization, RTCP handling, jitter buffer, and echo cancellation hooks.
...
- ✅ Work within this directory: `components/webrtc_integration/`
...
```python
# components/webrtc_integration/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "webrtc_integration"}
```
...
**Location**: Your work is in `components/webrtc_integration/`
...
```bash
python ../../orchestration/git_retry.py "webrtc_integration" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/webrtc_integration/

# Commit with component prefix
git commit -m "[webrtc_integration] feat: Add new capability"
...
```
[webrtc_integration] <type>: <description>
...
```bash
git status components/webrtc_integration/
# or
python ../../orchestration/git_status.py "webrtc_integration"
```
...
- [ ] Read API contract: `../../contracts/webrtc_integration-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "webrtc_integration" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "webrtc_integration" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/webrtc_integration-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/webrtc_integration
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(webrtc_integration): add input validation for common formats
...
**Commit Message Format:**
```
<type>(webrtc_integration): <subject>
