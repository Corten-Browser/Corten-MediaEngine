...
**Tech Stack**: Rust, widevine bindings, platform CDM interfaces
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the drm_support component.
...
### Purpose
DRM and Encrypted Media Extensions (EME). Implements CDM interface, EME API, license acquisition, secure decryption pipeline, and protected frame handling.
...
- ✅ Work within this directory: `components/drm_support/`
...
```python
# components/drm_support/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "drm_support"}
```
...
**Location**: Your work is in `components/drm_support/`
...
```bash
python ../../orchestration/git_retry.py "drm_support" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/drm_support/

# Commit with component prefix
git commit -m "[drm_support] feat: Add new capability"
...
```bash
git status components/drm_support/
# or
python ../../orchestration/git_status.py "drm_support"
```
...
- [ ] Read API contract: `../../contracts/drm_support-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "drm_support" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "drm_support" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/drm_support-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/drm_support
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(drm_support): add input validation for common formats
