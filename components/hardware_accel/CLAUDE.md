...
**Tech Stack**: Rust, va-rs (Linux), FFmpeg (cross-platform), platform-specific APIs
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the hardware_accel component.
...
### Purpose
Hardware-accelerated video decoding. Implements VA-API (Linux), DXVA (Windows), VideoToolbox (macOS) with automatic fallback to software decode and hardware capability detection.
...
- ✅ Work within this directory: `components/hardware_accel/`
...
```python
# components/hardware_accel/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "hardware_accel"}
```
...
**Location**: Your work is in `components/hardware_accel/`
...
```bash
python ../../orchestration/git_retry.py "hardware_accel" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/hardware_accel/

# Commit with component prefix
git commit -m "[hardware_accel] feat: Add new capability"
...
```
[hardware_accel] <type>: <description>
...
```bash
git status components/hardware_accel/
# or
python ../../orchestration/git_status.py "hardware_accel"
```
...
- [ ] Read API contract: `../../contracts/hardware_accel-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "hardware_accel" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "hardware_accel" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/hardware_accel-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/hardware_accel
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(hardware_accel): add input validation for common formats
